$(document).ready(function() {
  ...
  const exponent = modelMath.DEFAULT_EXPONENT;
  const maximumSupportedItemLevel = budgetModel.MAXIMUM_ITEM_LEVEL;
  ...
  const armorClass = {
    ...
    4: { name: "Shirt", sellMod: 4 / 16, armorMod: 0, itemClass: 4, subClass: [0] },
    5: { name: "Chest", sellMod: 16 / 16, armorMod: 16 / 16, itemClass: 4, subClass: [1, 2, 3, 4] },
    ...
    19: { name: "Tabard", sellMod: 4 / 16, armorMod: 0, itemClass: 4, subClass: [0] },
    20: { name: "Chest (Robe)", sellMod: 16 / 16, armorMod: 16 / 16, itemClass: 4, subClass: [1, 2, 3, 4] },
    23: { name: "Held Off-hand", sellMod: 8 / 16, armorMod: 0, itemClass: 4, subClass: [0] },
    28: { name: "Relic", sellMod: 4 / 16, armorMod: 0, itemClass: 4, subClass: [7, 8, 9, 10] }
  };

  const weaponClass = {
    13: { name: "One-Hand", sellMod: 7 / 16, armorMod: 0, itemClass: 2, subClass: [0, 4, 7, 15, 13] },
    15: { name: "Bow", sellMod: 16 / 16, armorMod: 0, itemClass: 2, subClass: [2] },
    17: { name: "Two-Hand", sellMod: 16 / 16, armorMod: 0, itemClass: 2, subClass: [1, 5, 8, 6, 10] },
    21: { name: "Main-Hand", sellMod: 7 / 16, armorMod: 0, itemClass: 2, subClass: [0, 4, 7, 15, 13] },
    22: { name: "Off-Hand", sellMod: 7 / 16, armorMod: 0, itemClass: 2, subClass: [0, 4, 7, 15, 13] },
    25: { name: "Thrown", sellMod: 5 / 16, armorMod: 0, itemClass: 2, subClass: [16] },
    26: { name: "Ranged", sellMod: 5 / 16, armorMod: 0, itemClass: 2, subClass: [3, 18, 19] }
  };
  ...
  const itemStats = {
    ...
    6: { name: "Spirit", type: 0 },
    7: { name: "Stamina", type: 0 },
    12: { name: "Defense Rating", type: 1 },
    13: { name: "Dodge Rating", type: 1 },
    14: { name: "Parry Rating", type: 1 },
    15: { name: "Block Rating", type: 1 },
    21: { name: "Spell Crit Rating", type: 1 },
    31: { name: "Hit Rating", type: 1 },
    32: { name: "Crit Rating", type: 1 },
    35: { name: "Resilience", type: 1 },
    36: { name: "Haste Rating", type: 1 },
    37: { name: "Expertise Rating", type: 1 },
    38: { name: "Attack Power", type: 1 },
    43: { name: "Mana Regen MP5", type: 1 },
    44: { name: "Armor Penetration Rating", type: 1 },
    45: { name: "Spell Power", type: 1 },
    46: { name: "Health Regen HP5", type: 1 },
    47: { name: "Spell Penetration", type: 1 },
    48: { name: "Block Value", type: 1 },
    arcane_res: { name: "Resist Arcane", type: 0 },
    fire_res: { name: "Resist Fire", type: 0 },
    nature_res: { name: "Resist Nature", type: 0 },
    frost_res: { name: "Resist Frost", type: 0 },
    shadow_res: { name: "Resist Shadow", type: 0 },
    meta_socket: { name: "Meta Socket", color: "meta", type: 2 },
    red_socket: { name: "Red Socket", color: "red", type: 2 },
    blue_socket: { name: "Blue Socket", color: "blue", type: 2 },
    yellow_socket: { name: "Yellow Socket", color: "yellow", type: 2 }
  };
  ...
  const weaponDPS = {
    ...
    3: {
      ...
      21: [ // Main-hand
        { type: 'caster', min: 1, max: 300, sub: [4, 7, 15], mod: lvl => 0.9195580731155433 + 0.7492658962645824 * lvl - 0.005319339993856751 * Math.pow(lvl, 2) + 0.000017593004422642952 * Math.pow(lvl, 3) },
  ...
  function calculateLevel(itemClass, slot, quality) {
    ...
    $('#stats .group').each(function() {
      stats.push({
        type: $(this).find('.stat-type').val(),
        amount: parseFloat($(this).find('.stat-amount').val())
      });
    });
...
const coefficient = uncommonDamage?.coefficient ?? (() => {
  ...
  return matchingEntry ? matchingEntry.mod : null;
    ...
    // Elemental bonus damage is rendered on its own line in WoW tooltips. It
    // must not also inflate the physical damage range or displayed physical DPS.
    const damageHTML = bonus ? bonus.display : '';
    ...
    const bonusDps = bonus
      ? ((bonus.min + (bonus.max ?? bonus.min)) / 2) /
        (attackSpeed / 1000)
      : 0;
  ...
  function updateStatGroup(type, action) {
    if (action == 'add') {
      ...
      if (type == 'socket' && socketCount <= 3) {
        const rowId = statRowId++;
        const statHtml = `
          <div class="group pill socket" id="socket-group-${rowId}">
            <input type="number" class="stat-amount hide" data-calc="" id="socket-amount-${rowId}" value="1" disabled />
            ${createSocketDropdown(`socket-${rowId}`)}
            <div class="delete"><i class="stage1 fa-solid fa-ellipsis-vertical"></i><i class="stage2 fa-regular fa-trash-can"></i></div>
          </div>`;
        $('#stats').append(statHtml);
  ...
  function calculateForm() { // form calculation
    ...
    if (calcMethod === 'stats') {
      const requestedLevel = Number($("#item-level").val());
      if (!Number.isInteger(requestedLevel) ||
          requestedLevel < 1 ||
          requestedLevel > maximumSupportedItemLevel) {
        $("#item-level").addClass('error');
        err = true;
      }
    ...
    let itemLevel = Number($('#item-level').val()) || null;
    let itemDescription = $('#item-description').val() || null;
    let statValue = null;
    const itemClass = Number($('#item-slot option:selected').data('class'));
    const itemBind = $('input[name="itemBind"]:checked').val() || null;
    const itemUnique = $('input[name="itemUnique"]:checked').val() || null;
    const itemQuality = Number($('input[name="itemQuality"]:checked').data('quality'));
    const itemSlot = Number($('#item-slot').val());
    const itemTypeKey = Number($('#item-subclass option:selected').val());
    ...
    if (itemClass == 4) { // armor properties
      itemArmor = calculateArmor(itemSlot, itemTypeKey, itemLevel, itemQuality, bonusArmor);
      if(itemSlot == 14) {
  ...
  $("#weaponSpeed").on('change input', function() {
    const obj = $(this);
    const val = Number(obj.val());
    if (val > 6) { obj.val(6); }
    if (obj.val() !== '' && val < 1) { obj.val(1); }
  });
  ...
  $('#item-level').on('change input', function() {
    ...
    else if (!Number.isInteger(level) || level < 1 || level > maximumSupportedItemLevel) {
      $(this).addClass('error');
    }
    else {
      $(this).removeClass('error');
  ...
  $('#item-subclass').on('change', function() {
    ...
    if (selectedClass == 2) { // is a weapon
      ...
      else if ($("#item-slot option:selected").val() == 15) {
        $('.weaponDamageExtra input').val('');
        $(".weaponDamageExtra").hide();
      }
